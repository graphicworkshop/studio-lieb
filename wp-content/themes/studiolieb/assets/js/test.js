  ...
  if ($j('.carousel').length) {
    ...
    if ($j('.carousel').data('parallax') == 'yes') {
      if ($j('.no-touch .carousel').length) {
        skrollr_slider = skrollr.init({edgeStrategy: 'set', smoothScrolling: true, forceHeight: false});
  ...
  if ($j('.qode_carousels').length) {
    $j('.qode_carousels').each(function () {
      ...
      $j(this).find('.slides').carouFredSel({
        ...
        items: {width: itemWidth, visible: {min: 1, max: 6}},
        auto: true,
        mousewheel: false,
...
function resizeMasonry(container) {
  ...
  container.isotope({masonry: {columnWidth: portfolio_width / parseInt($cols)}})
}
  ...
  if ($j('.q_masonry_blog').length) {
    $j('.q_masonry_blog').each(function () {
      ...
      $j(window).resize(function () {
        ...
        $container.isotope({masonry: {columnWidth: $container.width() / $cols}})
      });
...
function checkAnchorOnLoad() {
  ...
  if (hash !== "" && $j('[data-q_id="' + hash + '"]').length > 0) {
    ...
    $j('html, body').animate({scrollTop: $j('[data-q_id="' + hash + '"]').offset().top - $j('header.page_header').height()}, 1500, function () {
      ...
      if (currentHeaderHeight != initialHeaderHeight) {
        ...
        $j(this).animate({scrollTop: $j(this).scrollTop() + headerSizeDifference}, 1000)
      }
    })
  }
}
...
function initHashClick() {
  ...
  $j(document).on("click", ".main_menu a, .vertical_menu a, .qbutton, .anchor, nav.popup_menu ul li a", function () {
    ...
    if ((hash !== "" && $j(this).attr('href').split('#')[0] === "") || (hash !== "" && $j(this).attr('href').split('#')[0] !== "" && hash === window.location.hash) || ($j(this).attr('href').split('#')[0] == window.location.href.split('#')[0])) {
      ...
      if ($j('[data-q_id="' + hash + '"]').length > 0) {
        $doc.animate({scrollTop: $j('[data-q_id="' + hash + '"]').offset().top - initialHeaderHeight}, 1500, function () {
          currentHeaderHeight = $j('header.page_header').height();
          if (currentHeaderHeight != initialHeaderHeight) {
            var headerSizeDifference = initialHeaderHeight - currentHeaderHeight;
            $j(this).animate({scrollTop: $j(this).scrollTop() + headerSizeDifference}, 1000)
  ...
  $j(document).on("click", ".mobile_menu a", function () {
    ...
    if ((hash !== "" && $j(this).attr('href').split('#')[0] === "") || (hash !== "" && $j(this).attr('href').split('#')[0] !== "" && hash === window.location.hash) || ($j(this).attr('href').split('#')[0] == window.location.href.split('#')[0])) {
      if ($j('[data-q_id="' + hash + '"]').length > 0) {
        $doc.animate({scrollTop: $j('[data-q_id="' + hash + '"]').offset().top - $j('.mobile_menu').height()}, 500);
        anchorActiveState($j(this))
      }
      if (history.pushState) {
        history.pushState(null, null, hash)
